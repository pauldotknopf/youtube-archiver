...
namespace YouTubeArchiver.Index
{
    public class UpdateDb
    {
        ...
        public static async Task Run(string indexDirectory, string channelId)
        {
            ...
            if (string.IsNullOrEmpty(channelId))
            {
                Log.Logger.Error("You must provide a channel id.");
                Environment.Exit(1);
            }
        ...
        {
            var youtubeDumpAuthFile = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Personal), ".youtube-archiver-auth.json");
